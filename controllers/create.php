...
// Vérification du formulaire soumis => Commenté car apparaît dès la redirection vers le formulaire et non après sa soumission.
...
if ($_POST) {
    ...
    try {
        if (isset($_FILES['image']) && $_FILES['image']['size'] < 2000000 && $_FILES['image']['error'] === 0) {
            ...
            if (in_array(strtolower($fileExtension), $allowedExtensions) && !file_exists($targetFile)) {
